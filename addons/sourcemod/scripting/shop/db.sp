...
void DB_CheckTable3()
{
	...
	if (db_type == DB_MySQL)
	{
		h_db.Format(s_Query, sizeof(s_Query), "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = '%splayers';", g_sDbPrefix);
		DB_TQuery(DB_CheckTable4, s_Query);
	}
	else
	{
		h_db.Format(s_Query, sizeof(s_Query), "PRAGMA TABLE_INFO(%splayers);", g_sDbPrefix);
		DB_TQuery(DB_CheckTable4, s_Query);
	}
}
#endif
...
void DB_CreateTables()
{
	...
	if (db_type == DB_MySQL)
	{
		...
		h_db.Format(s_Query, sizeof(s_Query), "CREATE TABLE IF NOT EXISTS `%sboughts` (\
							  ...
							  `gold_buy_price` int NOT NULL,\
							  `sell_price` int NOT NULL,\
							  `gold_sell_price` int NOT NULL,\
							  `buy_time` int\
		...
		// Removed ENGINE=MyISAM for Google Cloud Platform
		h_db.Format(s_Query, sizeof(s_Query), "CREATE TABLE IF NOT EXISTS `%sitems` (\
							  `id` int NOT NULL AUTO_INCREMENT,\
							  `category` varchar(64) NOT NULL,\
							  `item` varchar(64) NOT NULL,\
							  PRIMARY KEY (`id`)\
							) DEFAULT CHARSET=" ... SHOP_MYSQL_CHARSET ... " AUTO_INCREMENT=1 ;", g_sDbPrefix);
	...
	else
	{
		...
		h_db.Format(s_Query, sizeof(s_Query), "CREATE TABLE IF NOT EXISTS `%sboughts` (\
							  ...
							  `gold_buy_price` INTEGER NOT NULL,\
							  `sell_price` NUMERIC NOT NULL,\
							  `gold_sell_price` NUMERIC NOT NULL,\
...
#if defined SHOP_CORE_PREMIUM
public void DB_CheckTable4(Database db, DBResultSet results, const char[] error, any data)
{
	if (error[0])
	{
		LogError("DB_CheckTable4: %s", error);
		delete h_db;
		h_db = null;
		CreateTimer(15.0, DB_ReconnectTimer);
		isLoading = false;
	...
	if (results.RowCount < 6)
	{
		DB_UpgradeToNewVersion2();
		return;
	}
...
void DB_UpgradeToNewVersion2()
{
	PrintToServer(ansi_red ... "[Shop] " ... ansi_bright_red ... "Started upgrading to version 3!" ... ansi_reset);

	char s_Query[256];

	if (db_type == DB_MySQL)
	{
		h_db.Format(s_Query, sizeof(s_Query), "ALTER TABLE `%sboughts` ADD COLUMN IF NOT EXISTS `gold_buy_price` int(10) NOT NULL DEFAULT '0' AFTER `buy_price`;", g_sDbPrefix);
		DB_TQueryEx(s_Query);
		h_db.Format(s_Query, sizeof(s_Query), "ALTER TABLE `%sboughts` ADD COLUMN IF NOT EXISTS `gold_sell_price` int(10) NOT NULL DEFAULT '0' AFTER `sell_price`;", g_sDbPrefix);
		DB_TQueryEx(s_Query);
		h_db.Format(s_Query, sizeof(s_Query), "ALTER TABLE `%splayers` ADD COLUMN IF NOT EXISTS `gold` int(10) NOT NULL DEFAULT '0' AFTER `money`;", g_sDbPrefix);
		DB_TQueryEx(s_Query);
	}
	else
	{
		h_db.Format(s_Query, sizeof(s_Query), "ALTER TABLE `%sboughts` ADD `gold_buy_price` INTEGER NOT NULL DEFAULT '0'", g_sDbPrefix);
		DB_TQueryEx(s_Query);
		h_db.Format(s_Query, sizeof(s_Query), "ALTER TABLE `%sboughts` ADD `gold_sell_price` NUMERIC NOT NULL DEFAULT '0'", g_sDbPrefix);
		DB_TQueryEx(s_Query);
		h_db.Format(s_Query, sizeof(s_Query), "ALTER TABLE `%splayers` ADD `gold` NUMERIC NOT NULL DEFAULT '0'", g_sDbPrefix);
		DB_TQueryEx(s_Query);
	}

	PrintToServer(ansi_red ... "[Shop] " ... ansi_bright_red ... "Upgrade to version 3 completed!" ... ansi_reset);

	DB_CreateTables();
}
#endif

void DB_AddQueryToQueue(const char[] query)
{
	queriesStack.PushString(query);
}

void DB_RunQueuedQueries()
{
	if (queriesStack.Empty || h_db == null)
	{
		return;
	}
	...
	{
		queriesStack.PopString(sQuery, sizeof(sQuery));
		trans.AddQuery(sQuery);
	}
	SQL_ExecuteTransaction(h_db, trans, _, DB_TranscationFailure);
}

void DB_TranscationFailure(Database db, any data, int numQueries, const char[] error, int failIndex, any[] queryData)
{
	if (error[0])
	{
		LogError("DB_TranscationFailure: %s, numQueries %d, failIndex: %d", error, numQueries, failIndex);
	}
}
